...
function get_graph_year_data($access_token,$account_id,$startYear,$endYear) //Api graph_year
{

    $curls = curl_init();
    curl_setopt_array($curls,array(
        CURLOPT_URL => 'https://zeustrare.my.salesforce.com/services/apexrest/DashboardAccountData/'.$account_id.'/?limitRecords=1000&selectedData=investAct&startYear='.$startYear.'&endYear='.$endYear,
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_CUSTOMREQUEST => 'GET',
        CURLOPT_HTTPHEADER => array(
            'Authorization: Bearer ' . $access_token,
...
$graph_year_data = get_graph_year_data($access_token,$_GET['account_id'],$_GET['startYear'],$_GET['endYear']);
